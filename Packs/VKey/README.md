...
The **V-Key V-OS Threat Intelligence** Content Pack enables seamless ingestion of mobile application security violations, device inventory profiles, and application package integrity telemetry from the **V-Key V-OS BSI (Business Security Intelligence)** API into **Cortex XSIAM**.
...
- **Interactive War Room Commands**: Enables SOC analysts to query recent telemetry directly using the `!vkey-get-threats` command.
...
- **Integrations**:
  - **VKey**: Connects to the V-Key V-OS Azure APIM BSI gateway and fetches threat, device, and application telemetry.
